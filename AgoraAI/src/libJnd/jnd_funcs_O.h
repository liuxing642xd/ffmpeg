...
template<int BLK_SZ_X, int BLK_SZ_Y> void blk_conv1_hwcn_avx(float *in_data, float *out_data, float *weight, float *bias)
{
	...
	for (int y = 0; y < BLK_SZ_Y + 4; y++)
	{
		for (int x = 0; x < BLK_SZ_X + 4; x += 4)
		{
			...
			vsum0 = _mm_max_ps(vsum0, vzero);
			vsum1 = _mm_max_ps(vsum1, vzero);
			vsum2 = _mm_max_ps(vsum2, vzero);
			vsum3 = _mm_max_ps(vsum3, vzero);

			_mm_storeu_ps(pOut + 0, vsum0);
			_mm_storeu_ps(pOut + 4, vsum1);
			_mm_storeu_ps(pOut + 8, vsum2);
			_mm_storeu_ps(pOut + 12, vsum3);
		}
	}
}
...
{
	__m128 vsum0, vsum1, vsum2, vsum3, vsum4, vsum5;
	...
	for (int y = 0; y < BLK_SZ_Y + 2; y++)
	{
		for (int x = 0; x < BLK_SZ_X + 2; x += 6)
		{
			...
			for (int kh = 0; kh < 3; kh++)
			{
				...
				for (int kw = 0; kw < 3; kw++)
				{
					__m128 vw = _mm_load_ps(pW + 4 * 0);
					vsum0 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 0), vw, vsum0);
					vsum1 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 4), vw, vsum1);
					vsum2 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 8), vw, vsum2);
					vsum3 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 12), vw, vsum3);
					vsum4 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 16), vw, vsum4);
					vsum5 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 20), vw, vsum5);

					vw = _mm_load_ps(pW + 4 * 1);
					vsum0 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 1), vw, vsum0);
					vsum1 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 5), vw, vsum1);
					vsum2 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 9), vw, vsum2);
					vsum3 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 13), vw, vsum3);
					vsum4 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 17), vw, vsum4);
					vsum5 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 21), vw, vsum5);

					vw = _mm_load_ps(pW + 4 * 2);
					vsum0 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 2), vw, vsum0);
					vsum1 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 6), vw, vsum1);
					vsum2 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 10), vw, vsum2);
					vsum3 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 14), vw, vsum3);
					vsum4 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 18), vw, vsum4);
					vsum5 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 22), vw, vsum5);

					vw = _mm_load_ps(pW + 4 * 3);
					vsum0 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 3), vw, vsum0);
					vsum1 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 7), vw, vsum1);
					vsum2 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 11), vw, vsum2);
					vsum3 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 15), vw, vsum3);
					vsum4 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 19), vw, vsum4);
					vsum5 = _mm_fmadd_ps(_mm_broadcast_ss(pInWin + 23), vw, vsum5);
			...
			vsum0 = _mm_max_ps(vzero, vsum0);
			vsum1 = _mm_max_ps(vzero, vsum1);
			vsum2 = _mm_max_ps(vzero, vsum2);
			vsum3 = _mm_max_ps(vzero, vsum3);
			vsum4 = _mm_max_ps(vzero, vsum4);
			vsum5 = _mm_max_ps(vzero, vsum5);

			_mm_storeu_ps(pOut + 0, vsum0);
			_mm_storeu_ps(pOut + 4, vsum1);
			_mm_storeu_ps(pOut + 8, vsum2);
			_mm_storeu_ps(pOut + 12, vsum3);
			_mm_storeu_ps(pOut + 16, vsum4);
			_mm_storeu_ps(pOut + 20, vsum5);
		}
	}
}
